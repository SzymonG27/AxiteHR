...
export class TempPasswordChangeComponent {
	...
	constructor(private blockUI: BlockUIService,
		...
		this.tempPasswordChangeForm = new FormGroup( {
			...
			confirmPassword: new FormControl(this.tempPasswordChangeModel.confirmPassword, {
				validators: [Validators.required, Validators.minLength(8), Validators.pattern(Environment.strongPasswordRegex), mustMatch('password')]
			})
		});
	}
	...
	async changeTempPassword() {
		...
		if (!tempPasswordUserId) {
			this.dataService.setTempPasswordError(await firstValueFrom(this.translate.get('Authentication_TempPasswordChange_InternalError')));
			this.blockUI.stop();
		...
		this.authService.TempPasswordChange(this.tempPasswordChangeModel).pipe(take(1)).subscribe({
			next: async (response: TempPasswordChangeResponse) => {
				if (response.isSucceeded) {
					this.dataService.setTempPasswordSuccess(await firstValueFrom(this.translate.get('Authentication_TempPasswordChange_Success')))
					this.blockUI.stop();
					this.router.navigate(['Login']);
				} else {
